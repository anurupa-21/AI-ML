...
--we dont want product column in the output but we want each month's total profit to be show by monthnumber & monthname ,Also a new column should be added to show next month's total profit
select monthnumber,monthname,sum(profit) [Total profit],lead(sum(profit)) over(order by monthnumber asc) [Next Month Total Profit] from ProfitData group by monthnumber,monthname order by MonthNumber;
