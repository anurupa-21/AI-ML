...
select max(PaymentMethod) [Max Pay Method] from Sales;

--Maximum quantity sold for each product Id
select productid,max(quantity) [Maximum Quantity] from sales group by ProductID;

--Maximum total amount for all distinct date in saledate column
select saledate,max(totalamount) [Maximum Total Amount] from Sales group by SaleDate;
